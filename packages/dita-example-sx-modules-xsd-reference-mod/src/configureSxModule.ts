...
export default function configureSxModule(sxModule) {
	...
	configureAsFrame(sxModule, xq`self::properties`, t('properties'), {
		...
		tabNavigationItemSelector: xq`name() = ("proptypehd", "propvaluehd", "propdeschd", "proptype", "propvalue", "propdesc")`,
			...
			columns: [
				{
					query: xq`./proptype`,
					width: 1,
					hideColumnIfQueryIsTrue: xq`parent::properties[not(child::property/proptype) and not(child::prophead/proptypehd)]`,
					clickOperationWhenEmpty: ':property-insert-proptype',
				},
				{
					query: xq`./propvalue`,
					width: 1,
					hideColumnIfQueryIsTrue: xq`parent::properties[not(child::property/propvalue) and not(child::prophead/propvaluehd)]`,
					clickOperationWhenEmpty: ':property-insert-propvalue',
				},
				{
					query: xq`./propdesc`,
					width: 1,
					hideColumnIfQueryIsTrue: xq`parent::properties[not(child::property/propdesc) and not(child::prophead/propdeschd)]`,
	...
	configureAsTitleFrame(
		sxModule,
		xq`self::title[parent::reference]`,
		undefined,
		{
